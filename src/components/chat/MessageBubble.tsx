...
export const MessageBubble = ({
  ...
  const myReactedSet = new Set(
    (reactions || []).filter((r) => r.user_id === myUserId).map((r) => r.emoji)
  );
  ...
  return (
    <div
      className={`relative mb-1 flex flex-col ${mine ? "items-end" : "items-start"}`}
    ...
    >
      ...
      {!isDeletedForEveryone && reactions && reactions.length > 0 && (
        <ReactionChips
          reactions={reactions}
          myUserId={myUserId}
          mine={mine}
